window.onload=function(){
	...
	//查看借还记录
	var onRecordBtn = document.getElementById('record');
	onRecordBtn.onclick = function()
	{
		var xmlhttp;
		if (window.XMLHttpRequest)
    	{
    	...
    	var url = 'http://localhost:8000/r';
    	xmlhttp.open('get', url, true);
    	xmlhttp.onreadystatechange = function()
		{
			if(xmlhttp.readyState == 4 && xmlhttp.status == 200)
			{
				document.getElementById('borrow_record').innerHTML = xmlhttp.responseText;
			}
		}
		xmlhttp.send();
	}
